    ...
    <div class="page-wrapper">
        <div class="page-content statistic_tab">
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3 index_header_handler">
                <div class="breadcrumb-title pe-3">{{$item->title}}</div>
                        ...
                        <div class="card-body">
                            <div class="card-title stat_card_sums">
                                ...
                                <div>
                                    ...
                                    <h3 class="should_money">{{$costs->where('date','>=',now()->firstOfMonth())->sum('value')}}</h3>
                                </div>
                                <div>
                                ...
                                <div>
                                    <h3 class="should_money">{{$costs->sum('value')}}</h3>
                                </div>
                                            ...
                                            <div class="form-group stat_item_select">
                                                ...
                                                <select   class="items_multiselect form-select select_2_select" name="items[]" multiple="multiple" id="filter_items_select">
                                                    ...
                                                    @foreach($item->subitems as $subitem)
                                                        <option value="{{$subitem->id}}">{{$subitem->title}}</option>
                            ...
                            <table id="small_table" class="table position_table table-striped table-bordered beautiful_overflow">
                                ...
                                @foreach($costs as $cost)
                                    <tr>
                                        ...
                                        <td>{{$cost->count}}</td>
                                        <td class="@if($cost->cashless === '1') cashless @endif">
                                        <span class="should_money">
                                            {{$cost->formatted_value}}
                                        </span>
                                        </td>
                                        @foreach($additionalColumns as $additional)
        ...
        $(document).ready(function (){
            ...
            $('#apply_filters').click(function (){
                ...
                $.ajax({
                    ...
                    url: "{{route('statistic.filter.category')}}"+"?"+$('#stat_form').serialize(),
                    data: {
            ...
            chart.render();
            let config = @json($chart_costs->toArray());
